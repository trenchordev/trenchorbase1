...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    // Poll job statuses and block number every 2 seconds (matches Base block time)
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex items-center gap-4">
            ...
            {currentBlock && (
              <div className="flex items-center gap-2 text-xs font-mono bg-[#00ff41]/10 px-3 py-2 rounded border border-[#00ff41]/30">
                <span className="opacity-50">CURRENT BLOCK:</span>
                <span className="text-[#00ff41] font-bold">{parseInt(currentBlock).toLocaleString()}</span>
                <button
                  onClick={fetchCurrentBlock}
                  className="ml-1 p-1 hover:bg-[#00ff41]/20 rounded-full transition-colors"
                  title="Refresh Block Number"
                >
                  <svg className="w-3 h-3 text-[#00ff41]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
