                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="bg-[#00ff41]/10 border-b border-[#00ff41]/30">
                          ...
                          <th className="px-2 py-2 text-right font-mono text-xs uppercase tracking-wider">VOLUME</th>
                      ...
                      <tbody>
                        {leaderboardData.length === 0 ? (
                          <tr><td colSpan="8" className="px-4 py-12 text-center opacity-50"><div className="text-4xl mb-2">📊</div><div>No trading data available</div></td></tr>
                        ) : (
                          displayedLeaderboard.map((entry) => {
                            const isTopThree = entry.rank <= 3;

                            const totalVirtual = Number(entry.totalVirtualVolume || 0);
                            const buyVirtual = Number(entry.buyVirtualVolume || 0);
                            const sellVirtual = Number(entry.sellVirtualVolume || 0);
                            const netVirtual = Number(entry.netBuyVirtual ?? 0);

                            const totalUsd = virtualPrice ? totalVirtual * virtualPrice : Number(entry.totalUsdVolume || 0);
                            const buyUsd = virtualPrice ? buyVirtual * virtualPrice : Number(entry.buyUsdVolume || 0);
                            const sellUsd = virtualPrice ? sellVirtual * virtualPrice : Number(entry.sellUsdVolume || 0);
                            const netUsd = virtualPrice ? netVirtual * virtualPrice : Number(entry.netBuyUsd || 0);
                              ...
                              <tr key={`${entry.address}-${entry.rank}`} className={`border-b border-white/5 hover:bg-[#00ff41]/5 transition-colors ${isTopThree ? 'bg-[#00ff41]/5' : ''}`}>
                                ...
                                <td className="px-2 py-2 font-mono text-right text-xs">
                                  <div className="flex flex-col items-end leading-tight">
                                    <span className="text-[#00ff41] font-bold">{formatUsd(totalUsd)}</span>
                                    <span className="text-cyan-400 text-[10px]">{formatVirtual(totalVirtual)}</span>
                                  </div>
                                </td>
                                <td className="px-2 py-2 font-mono text-right text-xs text-purple-400">{formatToken(entry.totalTokenVolume || 0)}</td>
                                <td className="px-2 py-2 font-mono text-right text-xs">
                                  <div className="flex flex-col items-end leading-tight">
                                    <span className="text-green-400">{formatVirtual(buyVirtual)}</span>
                                    <span className="text-white/60 text-[10px]">{formatUsd(buyUsd)}</span>
                                  </div>
                                </td>
                                <td className="px-2 py-2 font-mono text-right text-xs">
                                  <div className="flex flex-col items-end leading-tight">
                                    <span className="text-red-400">{formatVirtual(sellVirtual)}</span>
                                    <span className="text-white/60 text-[10px]">{formatUsd(sellUsd)}</span>
                                  </div>
                                </td>
                                <td className="px-2 py-2 font-mono text-right text-xs">
                                  <div className="flex flex-col items-end leading-tight">
                                    <span className={netVirtual >= 0 ? 'text-green-400' : 'text-red-400'}>{formatVirtual(netVirtual)}</span>
                                    <span className="text-white/60 text-[10px]">{formatUsd(netUsd)}</span>
                                  </div>
                                </td>
